...
- **"Subí el inventario y no aparece en el buscador" casi nunca es el inventario: es `farmacias.ubicacion`.** El registro (`auth.py:_INSERT_FARMACIA`) crea toda farmacia en `ST_MakePoint(0,0)` como marcador de "coordenadas pendientes", y el buscador filtra por `ST_DWithin` con radio máximo de 50 km — (0,0) queda a ~7 700 km de Acarigua, así que el 100% de su inventario es invisible. Hasta 2026-08-09 no existía ningún camino para corregirlo (el PATCH de Configuración no tocaba `ubicacion`), y solo las farmacias DEMO sembradas por SQL tenían coordenadas reales. Ahora `PATCH /api/v1/farmacias/{id}` acepta `lat`/`lng` (juntas o ninguna; una sola → 400) y el dashboard GET devuelve `lat`, `lng` y `ubicacion_configurada` para que el panel avise. Antes de depurar la carga, verificar la ubicación de la farmacia.
